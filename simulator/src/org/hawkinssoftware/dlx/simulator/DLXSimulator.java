...
public class DLXSimulator {
	...
	private static final int registerSnapshot[] = new int[32];

	static PrintStream simulatorOutput = System.out;

	static boolean echoInstructions = false;

	public static void main(String args[]) {
		try {
			int a = 0;
			if (args[a].equals("-e")) {
				echoInstructions(true);
				a++;
			}

			if ((args.length - a) != 1)
				throw new IllegalArgumentException("The DLX Simulator must have exactly one object file to execute!");
	...
	public static void echoInstructions(boolean b) {
		echoInstructions = b;
	}

	public static void load(String binaryFilename) throws IOException {
		File file = new File(binaryFilename);
		DataInputStream in = new DataInputStream(new FileInputStream(file));
		int instructions[] = new int[in.available() / 4];
		for (int i = 0; i < instructions.length; i++) {
			instructions[i] = in.readInt();
		}
		load(instructions);
		in.close();
	}
	...
	public static int getHeapValue(int location) {
		int index = (R[30] + location) / 4;
		if (index >= M.length)
			return -81; // recognizable value
		return M[index];
	}

	public static int[] getStackValues() {
		// calculating in words
		int stackStart = (R[30] / 4) - 1;
		int stackSize = stackStart - (R[29] / 4);
		int values[] = new int[stackSize];
		for (int i = 0; i < values.length; i++) {
			values[i] = M[stackStart - i - 1];
		}
		return values;
	}

	public static int[] getRegisterValues() {
		for (int i = 0; i < registerSnapshot.length; i++) {
			registerSnapshot[i] = R[i];
		}
	...
	public static void setSimulatorOutput(PrintStream simulatorOutput) {
		DLXSimulator.simulatorOutput = simulatorOutput;
	}
	...
	public static boolean step() throws IOException {
		try {
			...
			if (echoInstructions) {
				System.out.print(PC + "| " + mnemo[op] + " (" + a + ", " + b + ", " + c + ") | ");
				switch (format) {
					case 0:
						System.out.print("[c:" + c + "] | ");
						break;
					case 1:
						System.out.print("[a(R" + a + "):" + R[a] + "] [b(R" + b + "):" + R[b] + "] [c:" + c + "] | ");
						break;
					case 2:
						System.out.print("[a(R" + a + "):" + R[a] + "] [b(R" + b + "):" + R[b] + "] [c:R" + origc + ":" + R[origc] + "] | ");
						break;
				}
				System.out.print("[F:" + R[28] + "] [S:" + R[29] + "] [G:" + R[30] + "] [Ret:" + R[31] + "]");
				System.out.println();
			}
			switch (op) {
				case ADD:
				...
				case RET:
					if (origc == 0)
						return false; // break execloop; // remember: c==R[origc]
					if ((c < 0) || (c > MemSize)) {
				...
				case RDI:
					...
					break;
				case WRD:
					simulatorOutput.print(R[b] + "  ");
					break;
				case WRH:
					simulatorOutput.print("0x" + Integer.toHexString(R[b]) + "  ");
					break;
				case WRL:
					simulatorOutput.println();
					break;
				case ERR:
			...
			e.printStackTrace();
			System.out.println("failed at PC " + PC + ",   " + disassemble(M[PC]));
			return false;
		}
	}

	// Mnemonic-to-Opcode mapping
	static final String mnemo[] = { "ADD", "SUB", "MUL", "DIV", "MOD", "CMP", "ERR", "ERR", "OR", "AND", "BIC", "XOR", "LSH", "ASH", "CHK", "ERR", "ADDI",
			"SUBI", "MULI", "DIVI", "MODI", "CMPI", "ERRI", "ERRI", "ORI", "ANDI", "BICI", "XORI", "LSHI", "ASHI", "CHKI", "ERR", "LDW", "LDX", "POP", "ERR",
			"STW", "STX", "PSH", "ERR", "BEQ", "BNE", "BLT", "BGE", "BLE", "BGT", "BSR", "ERR", "JSR", "RET", "RDI", "WRD", "WRH", "WRL", "ERR", "ERR", "ERR",
